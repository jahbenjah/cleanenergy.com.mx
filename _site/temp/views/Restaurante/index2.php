<?php
	$newsDet= (isset($this->newsDetails[0])) ? $this->newsDetails[0] :  array('post_title' => 'Página no encontrada', 'post_image' => '', 'post_url' => '', 'post_date' => '', 'post_body' => 'La noticia no fue encontrada o fue eliminada.. <a href="'.URL.'">Ir al inicio</a>',);
	$newsRel=$this->news;
	$banner=$this->banner;
?>
<!DOCTYPE html>
...
<head>
	...
	<?php
		if (!empty($newsDet['post_url'])) {
			?>
			<meta property="og:video:url" 	content="<?= $newsDet['post_url'] ?>">
		    <meta property="og:video:type"  content="video/mp4">
		    <meta property="og:image" content="<?=str_replace('https://www.youtube.com/watch?v=', 'https://i1.ytimg.com/vi/', $newsDet['post_url'])?>/hqdefault.jpg" /> "   
		    <?php
		}
		else{
		?>
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-101728137-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments)};
  gtag('js', new Date());

  gtag('config', 'UA-101728137-1');
</script>
</head>
		...
		<div class="gutter-7px">
			<div id="main-content" class="col-md-8 fix-right">
				<article class="single-post">
					...
					<div class="showimg">
						<a href="http://chevroletbaja.com/" target="_blank"><img src="<?=URL.'public/img/chevrolet_banner.png'?>" alt="Chevrolet cavalier 2018" class="img-responsive"></a>
					</div>
					<hr class="line">
					<div class="entry-content text-justify" style="font-size: 1.4em;">
						<p><?= html_entity_decode($newsDet['post_body']) ?></p>
			...
			<div id="sidebar" class="col-md-4 fix-left">
				...
				<div class="widget wid-banner">
					<div class="content">
						<?php
							if (count($banner)==1) {
								?>
									<a href="http://chevroletbaja.com/" target="_blank"><img src="<?=URL?>public/img/banners/<?=$banner[0]['img']?>" class="img-responsive"/></a>
								<?php
							}
						?>
					</div>
				...
				<div class="widget wid-banner">
					<div class="content">
				        ...
				        <?php
				        	$bannerCarousel=$this->bannerCarousel;
				            for ($i=0; $i < count($bannerCarousel); $i++) {
				                ?>
				                	<div class="item">
				                        <a href="#">
				                            <div class="zoom-container">
				                                <div class="zoom-caption">
				                                    <div class="zoom-caption-inner hidden">
				                                        <span><?= $bannerCarousel[$i]['title']?></span>
				                                        
				                                        <h3><?= html_entity_decode($bannerCarousel[$i]['title'])?></h3>
				                                    </div>
				                                </div> 
				                                <img src="<?=URL?>public/img/banners/<?= $bannerCarousel[$i]['img']?>" /> 
				                            </div>
				                        </a>
				                    </div>
				                <?php
