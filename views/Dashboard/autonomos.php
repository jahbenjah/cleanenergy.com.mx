  ...
  <body>
    ...
    <div class="container">
      <div class="row"><span class="border-right">
        <div class="col-md-3">
          <a href="<?=URL?>dashboard/addProductAutonomos" class="btn btn-outline-info"><i class="fa fa-plus pr-1 text-info"></i> Agregar Producto</a>
        </div>
        </span>
        <div class="col-md-9">
              ...
              <table class="table table-striped table-sm">
                ...
                <?php
                    for ($i=0; $i < count($autonomos); $i++) { 
                      $linka= URL.'dashboard/editProductAutonomos/'.$autonomos[$i]["id"];
                      $linka.="/".str_replace(" ", '-', html_entity_decode($autonomos[$i]['nombre']));
                        echo "
                            <tr>
                              <td id='nombre-".$autonomos[$i]['id']."' ><a href='$linka' class='text-info'>".$autonomos[$i]['nombre']."</a></td>
                               <td>".$autonomos[$i]['categoria']."</td>
                               <td>".$autonomos[$i]['fabricante']."</td>
                               <td>".'<button type="button" class="btn btn-sm btn-outline-secondary" data-toggle="modal" data-target="#modal-business" onclick="delBusinessTEMP('.$autonomos[$i]['id'].')"><i class="fa fa-trash"></i></button>'."</td>
                              <td></td>
